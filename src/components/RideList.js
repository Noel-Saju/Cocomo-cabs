      ...
      return rideRequests.map(request => (
        <div className="ride-list__result-item" key={request.rideUuid}>
          ...
          <div>
            ...
            <p>Estimated Earnings: Rs.{Math.floor(haversine({latitude: request.pickup.x, longitude: request.pickup.y}, {latitude: request.destination.x, longitude: request.destination.y})/1000)*15}</p>
